let currentUser = null; 
        ...
        try {
            ...
            if (!storedUserData) {
              //  console.log("Auth Check (Head): No user data found in localStorage. Redirecting to login...");
            ...
            if (!currentUser || !currentUser.username || !currentUser.id) {
                 console.error("Auth Check (Head): Invalid user data format in localStorage.");
                 localStorage.removeItem('userData');
                 if (!window.location.pathname.endsWith('login.html')) { window.location.replace('login.html'); }
                 throw new Error("Invalid user data.");
             }
           // console.log("Auth Check (Head): Authentication successful for user:", currentUser.username, "ID:", currentUser.id);
